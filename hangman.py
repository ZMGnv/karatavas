...
while run:
    ...
    def close():
        global run
        answer = messagebox.askyesno('Uzmanību!','Vai vēlaties beigt spēli?')
        if answer == True:
            run = False
            root.destroy()
            
    e1 = PhotoImage(file = 'exit2.png')
    ex = Button(root,bd = 0,command = close,bg="#E7FFFF",activebackground = "#E7FFFF",font = 10,image = e1)
    ex.place(x=770,y=10)
    s2 = 'Rezultāts:'+str(score)
    ...
    def check(letter,button):
        ...
        if letter in selected_word:
            ...
            if win_count == len(selected_word):
                score += 1
                answer = messagebox.askyesno('Spēle beigusies','Tu vinnēji!\nVēlies atkārtot?')
                if answer == True:
                    run = True
                    root.destroy()   
                else:
            ...
            if count == 6:
                answer = messagebox.askyesno('Spēle beigusies','Tu zaudēji!\nWANT TO Vēlies atkārtot?')
                if answer == True:
                    run = True
                    score = 0
                    root.destroy()
